...
const wrap = (kind: string, s: any, patient: Patient, body: string, docDate?: string, docMeta?: string) => {
  ...
  const initial = (s.name || 'D').charAt(0).toUpperCase();
  return `<html><head><meta charset="utf-8"/><title>${kind} — ${patient.fullName}</title><style>
    *{box-sizing:border-box}
    ...
    .logo{width:44px;height:44px;border-radius:10px;background:#0f766e;color:#fff;font-weight:800;font-size:22px;display:flex;align-items:center;justify-content:center}
    .cname{font-size:20px;font-weight:800;color:#0f766e;line-height:1.1}
    .muted{color:#64748b;font-size:12px}
    .doc{text-align:right}.doc .k{font-size:20px;font-weight:800;letter-spacing:.06em;color:#0f172a;text-transform:uppercase}
    .rule{height:3px;background:#0f766e;border-radius:3px;margin:14px 0 18px}
    ...
    <div class="head">
      <div class="brand"><div class="logo">${initial}</div><div>
        <div class="cname">${s.name || 'Dental Clinic'}</div>
        <div class="muted">${s.address || ''}${s.phone ? ' · ' + s.phone : ''}</div></div></div>
      <div class="doc"><div class="k">${kind}</div><div class="muted">${docMeta ? docMeta + '<br/>' : ''}Date: ${dDate(docDate || new Date().toISOString())}</div></div>
    </div>
    <div class="rule"></div>
    <div class="bill">
    ...
    ${body}
    <div class="foot">Generated by ${s.name || 'Dental Manager'} · Thank you for choosing us.</div></body></html>`;
};
